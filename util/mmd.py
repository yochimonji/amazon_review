...
def run_test(embed: torch.nn.Module, classifier: torch.nn.Module, iter: data.BucketIterator, device: torch.device):
    total_accuracy = 0
    total_f1 = 0
    embed.eval()
    classifier.eval()
    for batch in iter:
        ...
        with torch.no_grad():
            embed_sentence = embed(x)
            pred = classifier(embed_sentence)
